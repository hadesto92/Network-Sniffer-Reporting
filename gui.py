...
from main import save_conf, load_conf, start_scheduler, stop_scheduler, start_scheduler_solo, generuj_raport_pdf, \
    LOG_FILE, ARCHIWUM_FILE, wczytaj_archiwum, zapisz_archiwum
...
class App:
    def __init__(self, root):
        ...
        tk.Button(root, text="Edytuj dane", command=self.edit_host).grid(row=9, column=1, pady=5)
    ...
    def edit_host(self):
        ...
        def save_edit_host():
            self.date.get(combo.get(), {})["name_host"] = self.entry_edited_host_name.get()

            zapisz_archiwum(self.date)
            win_edit_host.destroy()

        combo.bind("<<ComboboxSelected>>", load_data)
        load_data()

        tk.Button(frame_edit_host, text="Zapisz", command=lambda: save_edit_host()).grid(row=5, column=0)
        tk.Button(frame_edit_host, text="Pokaż listę IP", command=lambda: self.show_ip_list(self.date.get(combo.get(), {}).get("ips"), combo.get())).grid(row=5, column=1)

    def show_ip_list(self, ip_list, host):
        win_ip_host = tk.Toplevel(self.root)
        win_ip_host.title(f"Lista adresów IP hosta {host}")
        win_ip_host.geometry("200x200")
    ...
    def setting_email(self):
        ...
        tk.Label(frame_email, text="Hasło/Hasło bezpieczeństwa:").grid(row=2, column=0, sticky="w")
        ...
        tk.Label(frame_email, text="SMTP server:").grid(row=3, column=0, sticky="w")
        self.entry_smtp_server = tk.Entry(frame_email, width=30)
        self.entry_smtp_server.grid(row=3, column=1)
        self.entry_smtp_server.insert(0, self.conf["smtp_server"])

        tk.Label(frame_email, text="SMTP port:").grid(row=4, column=0, sticky="w")
        self.entry_smtp_port = tk.Entry(frame_email, width=30)
        self.entry_smtp_port.grid(row=4, column=1)
        self.entry_smtp_port.insert(0, self.conf["smtp_port"])

        tk.Label(frame_email, text="SMTP TLS:").grid(row=5, column=0, sticky="w")
        checkbutton_value = tk.BooleanVar(value=True)
        self.entry_smtp_tls = tk.Checkbutton(frame_email, variable=checkbutton_value)
        self.entry_smtp_tls.grid(row=5, column=1)

        tk.Button(frame_email, text="Zapisz", command=lambda: save_email()).grid(row=7, column=1)

        def save_email():
            self.conf["sender_email"] = self.entry_sender_email.get()
            self.conf["sender_password"] = self.entry_sender_password.get()
            self.conf["smtp_server"] = self.entry_smtp_server.get()
            self.conf["smtp_port"] = self.entry_smtp_port.get()
            if checkbutton_value.get():
                self.conf["smtp_tls"] = True
            else:
                self.conf["smtp_tls"] = False
    ...
    def show_help(self):
        help_text = (
            ...
            " -> Generowanie klucza jest jedno razowe na wszystkie pliki. \n"
            "    Aby wygenerować ponownie klucz należy usunąć pliki takie jak: \n"
            "    => archive.json \n"
            "    => host.json \n"
            "    => log.json \n"
            "    => secret.key \n"
