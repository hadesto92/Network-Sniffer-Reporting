...
from cryptokey import write_encrypted_json, read_encrypted_json
...
def wczytaj_historie():
    return read_encrypted_json(HISTORIA_FILE)

def zapisz_historie(historia):
    write_encrypted_json(HISTORIA_FILE, historia)

def wczytaj_archiwum():
    return read_encrypted_json(ARCHIWUM_FILE)
...
def zapisz_archiwum(archiwum):
    write_encrypted_json(ARCHIWUM_FILE, archiwum)
